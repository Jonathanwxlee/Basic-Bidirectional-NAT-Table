...
NatTableEntry* NatTable_fwd_lookup(const NatTable* nat_table, const FiveTuple* fwd_key) {
    ...
    while (entry) {
        ...
        if (FiveTuple_compare(entry->fwd_key, *fwd_key)) return entry;
...
NatTableEntry* NatTable_rev_lookup(const NatTable* nat_table, const FiveTuple* rev_key) {
    ...
    while (entry) {
        if (FiveTuple_compare(entry->rev_key, *rev_key)) return entry;
...
int NatTable_remove(NatTable* nat_table, const FiveTuple* fwd_key) {
    ...
    uint32_t gateway_ip; // NULL initialization until we get find the gateway
    uint16_t gateway_port;
    ...
    if (FiveTuple_compare(entry->fwd_key, *fwd_key)) {
        fwd_to_free = entry;
        ...
        while (entry) {
            if (entry->next_fwd_entry) {
                if (FiveTuple_compare(entry->next_fwd_entry->fwd_key, *fwd_key)) {
    ...
    if (FiveTuple_compare(entry->rev_key, rev_key)) {
        rev_to_free = entry;
        ...
        while (entry) {
            if (entry->next_rev_entry) { // this outer if conditional is just ot make sure we don't get into a seg fault on the inside. 
                if (FiveTuple_compare(entry->next_rev_entry->rev_key, rev_key)) {
